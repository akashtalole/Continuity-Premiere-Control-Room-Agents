...
async def inject_anomaly(request: InjectAnomalyRequest) -> InjectAnomalyResponse:
    ...
    incident_id = await orchestrator.start_incident(anomaly)
    return InjectAnomalyResponse(incident_id=incident_id)


@router.post("/chaos", response_model=ChaosResponse)
async def trigger_chaos(request: ChaosRequest) -> ChaosResponse:

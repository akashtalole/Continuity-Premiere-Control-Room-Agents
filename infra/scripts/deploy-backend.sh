...
# defaults run the mock crew against SQLite):
...
#   GOOGLE_API_KEY                    stored in Secret Manager; prompted if unset and interactive
...
#
# Leaving GOOGLE_API_KEY / GRAFANA_URL unset deploys the backend running the
# deterministic mock crew -- see docs/agents.md -- which is a legitimate,
# fully-functional way to get a live demo URL without Grafana/Gemini
# credentials on hand yet.
...
prompt_secret GOOGLE_API_KEY "Gemini API key (leave blank to run the mock crew)"
...
log "Deploying $BACKEND_SERVICE to Cloud Run ($REGION)"
gcloud "${DEPLOY_ARGS[@]}"
